import 'package:paginate_demo/src/pagination/cursor_pagination.dart';
...
/// with `keepAlive: false`
///
mixin AutoDisposeCursorPaginationControllerMixin<T>
    // ignore: invalid_use_of_internal_member
    on BuildlessAutoDisposeAsyncNotifier<CursorPaginationResponse<T>> {
  /// Load data
  FutureOr<CursorPaginationResponse<T>> loadData(
    CursorPaginationRequest? query,
  );
  ...
  /// ```
  CursorPaginationRequest nextPage(CursorPaginationResponse<T> current);

  Future<void> loadMore() async {
    final oldState = state;
    if (oldState.requireValue.isCompleted) return;
    state =
        AsyncLoading<CursorPaginationResponse<T>>().copyWithPrevious(oldState);
    state = await AsyncValue.guard<CursorPaginationResponse<T>>(() async {
...
/// For riverpod async notifier [AsyncNotifier]
/// with `keepAlive: true`
mixin CursorPaginationControllerMixin<T>
    // ignore: invalid_use_of_internal_member
    on BuildlessAsyncNotifier<CursorPaginationResponse<T>> {
  /// Load data
  FutureOr<CursorPaginationResponse<T>> loadData(
    CursorPaginationRequest? query,
  );
